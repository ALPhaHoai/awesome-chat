$(document).ready(function () {
    ...
    $('#setNick').submit(function () {
        ...
        // }else{
        socket.emit('new user', (window.username = $('#nickName').val()), function (data) {
            if (data) {
    ...
    function usernameClick() {
        $('.user').click(function () {
            let friend = $(this).text();
            if (friend != window.username) {
                console.log('Hello ' + friend);

                $('.msg_box').show();
                $('#box_name').html(friend);
                socket.emit('open-chatbox', friend);
            }
        });
    }
